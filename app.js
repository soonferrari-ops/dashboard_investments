...
function applyI18n() {
  ...
  if (dashEmptyText) dashEmptyText.innerHTML = t('semAtivos') + '<br><a href="#" class="link" data-page="adicionar" id="dash-empty-link">' + t('adicionaPrimeiro') + '</a>';
  ...
  // Ativos page title (when rendered)
  const ativosTitle = document.getElementById('ativos-title');
  if (ativosTitle && ativosTitle.textContent === 'Ativos') ativosTitle.textContent = t('ativos');
  ...
  if (lastUpdate && lastUpdate.textContent.startsWith('Atualizado')) {
    const time = lastUpdate.textContent.split(': ')[1] || '';
    lastUpdate.textContent = t('atualizadoLabel') + ' ' + time;
  }
}
